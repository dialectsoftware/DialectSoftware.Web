...
namespace DialectSoftware.Web.HtmlAgilityPack
{
	/// <summary>
	...
	public class HtmlNode: IXPathNavigable
	{
		...
		internal void WriteAttribute(TextWriter outText, HtmlAttribute att)
		{
			...
			if (_ownerdocument.OptionOutputAsXml)
			{
				...
				{
                    if( _ownerdocument.OptionOutputPreserveStructure == false)
                    {
		...
		public void WriteTo(TextWriter outText)
		{
			...
			switch(_nodetype)
			{
				case HtmlNodeType.Comment:
					...
					if (_ownerdocument.OptionOutputAsXml)
					{
                        if(_ownerdocument.OptionOutputPreserveStructure)
                        {
						    outText.Write(((HtmlCommentNode)this).OuterHtml);
                        }
                        else
                        {
						...
						if (_ownerdocument.DocumentNode.HasChildNodes)
						{
							...
							if (rootnodes > 0)
							{
								...
								if (rootnodes > 1)
								{
                                    ...
                                    else if(_ownerdocument.OptionOutputPreserveStructure)
                                    {
                                        WriteContentTo(outText);
                                    }
									else
